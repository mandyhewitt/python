...
int normalize (float v1[], float v2[]) {
    float fMag;
...
// todo I won't need argc and argv anymore but I will need costheta and the photon input/output vectors from p and pp
// todo is pp a copy of p (by this stage) or a new photon direction?
{
    int i, j, n, count, mainloop, r1, c1, r2, c2, k, errno;
...
//    int rand(void);                                       // not needed here
    ...
    float adotb2, adotb3;                   // dotproducts
    float adotb, prod1, prod2, prod3, magnitude_a, magnitude_b, costheta, Iout, dotResult;
    float cosphi, sinphi;
    float phi_dot_result;
    float magnitude_r_obs;                  // magnitude of r axis in observer Frame of Reference
    float magnitude_r_out;                  // magnitude of r axis in scattering Frame of Reference
    float mag_mag;                          //
    float cross_mag;                        //
    float StokesVector[4][4] = {};          // Stokes vector
    float RotationVector[3][3] = {};        // Rotation vector
    float StokesOut[4][1] = {};             // Stokes matrix scattered photon
    float StokesObs[3][1] = {};             // Stokes matrix scattered photon observer frame of reference
    float StokesIn[4][1] = {};              // Stokes Matrix for incoming photon
    float vect1[3];                         // n_in scattering plane vector
    float vect2[3];                         // n_out scattering plane vector
    float vectl[3];                         // l_scattering plane un-normalized vector
    float vectResult[3];                    // generic cross product result vector
    float vectrObsResult[3];                // unnormalized r_observer frame vector
    float vectlObsResult[3];                // unnormalized l_observer frame vector
    float vectNormal[3];                    // normalized r_scattering plane vector
    float vectlNormal[3];                   // normalized l_scattering plane vector
    float vectrObsNormal[3];                // normalized r_observer frame vector
    float vectlObsNormal[3];                // normalized l_observer frame vector
    float zaxis[3] = {0.0, 0.0, 1.0};       // z axis is the reference axis for our observer
    float xaxis[3] = {1.0, 0.0, 0.0};       // x axis is the alternative reference axis for our observer
    float vectxprod[3];                     // cross product r_obs and r_out to calculate sinphi
    float sinphi_numerator;                 // (r_obs x r_s).n_obs
...
//todo need to capture the actual intensity
...
// todo we are going to write the q and u values to the photon information - work out how to update the structure
// todo the writing to file will also happen elsewhere in python
...
*/
        ...
        if (magnitude_a && magnitude_b) {
            ...
            // TODO identify incoming and outgoing direction from p and pp
            ...
            if (n == 0) {
                ...
                if (m == 0) {
                    ...
                    printf("Set reference axis as z axis.\n");              // input and output vectors are the z axis
                }

            } else {

                // todo use the correct crossproduct routine from python
                crossProduct(vect1, vect2, vectResult);                     // pol
            ...
            if (normalize(vectResult,vectNormal) != 0) {
                ...
                printf("ERROR in r scattering normalize function\n");
                printf("ERROR Input and output vectors are anti-parallel\n");
                printf("ERROR choose any perpendicular vector to output vector to be the r scattering axis\n");
            ...
            crossProduct(vect2, vectResult, vectl);
            ...
            if (normalize(vectl,vectlNormal) != 0) {
                printf("ERROR in l scattering normalize function\n");
                return 1;                                   // TODO what error codes should I be using?check python
            }
            ...
            // todo use python crossproduct routine

            if (ref_axis == 0) {
                crossProduct(vect2, zaxis, vectrObsResult);
            } else {
                crossProduct(vect2, xaxis, vectrObsResult);
            }
            ...
            if (normalize(vectrObsResult,vectrObsNormal) != 0) {
                printf("ERROR in r observer frame normalize function\n");
                return 1;                                   // TODO what error codes should I be using?check python
            }
            ...
            //todo add error checking USE PYTHON crossproduct routine

            crossProduct(vect2, vectrObsNormal, vectlObsResult);
            ...
            if (normalize(vectlObsResult,vectlObsNormal) != 0) {
                printf("ERROR in l observer frame  normalize function\n");
                return 1;                                   // TODO what error codes should I be using?check python
            }
            ...
            costheta = adotb / (magnitude_a * magnitude_b);
            ...
            //**************************POLARIZATION DEGREE*****************************************************
            /* Now we have calculated the direction of propagation of the photon and 2 vectors: inwards and outwards
            ...
            printf("Random wave intensity: \"%f\".\n", intensity);
            ...
            //cos phi is the dotproduct of r_obs and r_out divided by the magnitude of the 2 vectors
            ...
            if (magnitude_r_obs == 0) {
                ...
                printf("ERROR Null magnitude for r vector in observer frame of reference \n");
                return 1;                                   // TODO what error codes should I be using?check python
            }
            ...
            if (magnitude_r_out == 0) {
                ...
                printf("ERROR Null magnitude for r vector out in photon frame of reference \n");
                return 1;                                   // TODO what error codes should I be using?check python
            }
            ...
            phi_dot_result = dotProduct(vectResult, vectrObsResult);
            ...
            crossProduct(vectrObsResult, vectResult, vectxprod);
            ...
            sinphi_numerator = dotProduct(vectxprod,vect2);
            ...
            */
            ...
            printf("Error: A vector magnitude is zero. Unable to calculate Thomson scattering angle "
                           "or Polarization degree\n");
            //TODO: what do we put in the file when we hit an error?
        }
    }
    // end of main loop
